...
async function main() {
  ...
  if (!files.length) {
    throw new Error(
      "No CSS files found in package `@radix-ui/colors`. It is probably broken after updating the package.",
    );
  }

  const radixCssMatchingRegex = /^--(\w+)-(\w+):\s*(#\w+);$/;
  const radixP3MatchingRegex =
    /^--(\w+)-(\w+):\s*color\(display-p3 (\d+\.?\d*) (\d+\.?\d*) (\d+\.?\d*)(?: \/ (\d+\.?\d*))?\);$/;
    ...
    for (const line of lines) {
      ...
      if (leadingSpaces.length <= 2) {
        ...
        if (!matches || matches.length < 4) {
          // Check if the line is a valid color line.
          newLines.push(line);
          continue;
        }
        ...
        if (!colorName || !colorIndex || !colorValue) {
  ...
  // --- Start: New logic to generate combined CSS file for Tailwind v4 ---
  const combinedCssLines: string[] = ["@theme {"];
  for (const currentFilename of files) {
    // We only care about the standard hex values for the combined file
    const currentFilePath = path.join(radixRelativePackagePath, currentFilename);
    const currentFile = Bun.file(currentFilePath);
    const currentText = await currentFile.text();
    ...
    for (const line of lines) {
      const leadingSpaces = line.match(/^\s+/)?.[0] ?? "";
      // We only process the root hex definitions (ignore @media, P3 etc. for this combined file)
      if (leadingSpaces.length > 2) continue;
      const lineWithoutLeadingSpaces = line.replace(/^\s+/, "");
      const matches = lineWithoutLeadingSpaces.match(radixCssMatchingRegex);
      ...
      if (matches && matches.length >= 4) {
        ...
        if (colorName && colorIndex && colorValue) {
          const convertedColorValue = hexToRgb(colorValue);
          const newLine = `  --color-${colorName}-${colorIndex}: ${convertedColorValue};`;
          combinedCssLines.push(newLine);
        }
      }
    }
  }
  combinedCssLines.push("}"); // Close @theme

  const combinedFilePath = path.join(outputDirPath, "radix-colors.css");
  // Add header comment before the content
  const finalCombinedContent = headerComment + "\n\n" + combinedCssLines.join("\n") + "\n";
  await Bun.write(combinedFilePath, finalCombinedContent);
  // --- End: New logic ---
}
